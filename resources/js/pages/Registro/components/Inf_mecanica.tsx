...
interface Inf_mecanicaProps {
    onSubmit?: (data: { sistemas: SistemaInspeccion[] }) => void;
    onCancel?: () => void;
    evaluacion_mecanica?: any;
}
...
export default function Inf_mecanica({ onSubmit, onCancel, evaluacion_mecanica }: Inf_mecanicaProps) {
    ...
    useEffect(() => {
        if (evaluacion_mecanica && Array.isArray(evaluacion_mecanica) && evaluacion_mecanica.length > 0) {
            const datosIniciales: InspeccionData = {};

            evaluacion_mecanica.forEach((item: any) => {
                datosIniciales[item.componente] = {
                    nombre_sistema: item.nombre_sistema,
            ...
            setDatosInspeccion(datosIniciales);
        }
    }, [evaluacion_mecanica]);
