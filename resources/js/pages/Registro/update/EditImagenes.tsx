...
import Inf_Imagenes from '../components/Inf_Imagenes';
...
export default function EditImagenes({ id, imagenes }: { id: number, imagenes: any }) {
    ...
    const handleSubmit = (imagenes: any) => {
        ...
        imagenes.forEach((imagen: any, index: number) => {
            // Si la imagen tiene ID, es una imagen existente
            if (imagen.id && imagen.isExisting) {
                formData.append(`imagenes[${index}][id]`, imagen.id);
            }

            // Si la imagen tiene file, es una imagen nueva
            if (imagen.file) {
                formData.append(`imagenes[${index}][file]`, imagen.file);
            }

            // Siempre enviar ubicacion y descripcion
        ...
        router.post(route('imagenes.vehiculo.update', id), formData, {
            forceFormData: true,
