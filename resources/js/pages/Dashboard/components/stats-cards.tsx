            ...
            {stats.map((stat, index) => (
                <Card key={index} className="border-[#e2e8f0] bg-white transition-all hover:border-[#00AEEF] hover:shadow-lg dark:border-[#20384b] dark:bg-[#1a2c3a] dark:hover:border-[#00AEEF]">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-[#64748b] dark:text-white/70">
                            {stat.title}
                        </CardTitle>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-[#1e293b] dark:text-white/90 sm:text-3xl">{stat.value}</div>
                        <div className="mt-2 flex items-center gap-1 text-xs">
                            {stat.alert ? (
                                <>
                                    <AlertTriangle className="h-3 w-3 text-orange-500 dark:text-orange-400" />
                                    <span className="text-orange-500 dark:text-orange-400">{stat.trend}</span>
                                </>
                            ) : stat.trendUp ? (
                                <>
                                ...
                                <span className="text-[#64748b] dark:text-white/70">{stat.trend}</span>
                            )}
                        </div>
                    </CardContent>
                </Card>
